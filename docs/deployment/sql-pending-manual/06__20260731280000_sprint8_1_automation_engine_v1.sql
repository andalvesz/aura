...
-- Extend settings (safe defaults) — only if Aura Brain Core already applied
do $$
begin
  if to_regclass('public.aura_brain_settings') is not null then
    ...
    raise exception 'Prerequisite missing: public.aura_brain_settings. Run docs/deployment/sql-manual/03__20260728180000_aura_brain_core_v1.sql first.';
  end if;
end $$;
...
drop policy if exists "aura_automations_select_own" on public.aura_automations;
...
drop policy if exists "aura_automations_insert_own" on public.aura_automations;
...
drop policy if exists "aura_automations_update_own" on public.aura_automations;
...
drop policy if exists "aura_automation_attempts_select" on public.aura_automation_attempts;
...
drop policy if exists "aura_automation_confirmations_select" on public.aura_automation_confirmations;
...
drop policy if exists "aura_automation_schedules_select" on public.aura_automation_schedules;
...
drop policy if exists "aura_automation_leases_select" on public.aura_automation_leases;
...
drop policy if exists "aura_automation_audit_select_own" on public.aura_automation_audit;
create policy "aura_automation_audit_select_own"
  on public.aura_automation_audit for select
  using (auth.uid() = user_id);

drop policy if exists "aura_automation_audit_insert_own" on public.aura_automation_audit;
